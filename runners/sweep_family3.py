...
def run(job):
    ...
    return dict(dataset=spec[1], family=spec[0], rep=rep, fam="knn2ec",
                true_k=tk, k_hat=r.k, k_init=r.k_init, m_edges=r.n_tests,
                cand_connected=True, n_tests=r.n_tests,
                secs=round(time.time() - t0, 2))

jobs = [(s, r) for s in NULL_SPECS for r in range(5)]
jobs += [(s, r) for s in STRUCT_SPECS for r in range(3)]
if __name__ == "__main__":
    with Pool(2) as p:
        rows = p.map(run, jobs)
    pd.DataFrame(rows).to_csv("family_sweep3.csv", index=False)
    print("done")
